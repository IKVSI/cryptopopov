...
Защита от перехвата секретного сообщения на основе проблемы дискретного логорифмирования.

## Описание протокола

    1) A выбирает число e взаимно простое с p-1. Также существует число d такое, что de=1 mod (p-1).
    2) A шифрует сообщение M, с помощью функции E(e,M)=M^{e}mod(p)
    3) Получатель B аналогично выбирает целое число b, взаимно простое с p-1, и число с такое, что cb=1 mod (p-1).
    4) Вычисляет E(b,E(e,M))=(M^{e})^{b}mod(p) и отправляет обратно.
    5) Сторона A, получив сообщение, вычисляет D(d,M^{eb})=(M^{eb})^{d}=M^{b(1+n(p-1))}=M^{b}M^{n(p-1)b}=M^{b}mod(P)}
    6) A отправляет получателю M^{b}mod(P).
    7) B расшифровывает сообщение: D(c,M^{b})=M^{bc}mod(p)=M
